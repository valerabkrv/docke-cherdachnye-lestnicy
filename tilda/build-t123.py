#!/usr/bin/env python3
"""
Сборка сайта «Чердачные лестницы Döcke» в код для блока T123 (HTML-код) Тильды.

    python3 tilda/build-t123.py index.html --out tilda/lestnicy-t123.html \
        --scope dk --base https://valerabkrv.github.io/docke-cherdachnye-lestnicy/
...
def build(src_path, scope, base):
    ...
    #    ни STAIRS, ни renderCatalog не появлялись). Поэтому теги вырезаем,
    ...
    note = ('<!-- Чердачные лестницы Döcke — блок T123 для Тильды.\n'
...
def check(out, scope, base):
    ...
    for need in ('function load(', 'function start(', 'renderCatalog'):
        if need not in out:
